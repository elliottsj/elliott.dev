...
- The CLI has some odd ergonomics.
  ...
  The final command uploads the edited source code to the deployed "dev" environment but doesn't update the "dev" _Sandbox_ automatically. To bring your changes back into the Sandbox so it can be edited in the online IDE, you need to _tear down_ the existing Sandbox and "Fork" the dev _environment_ into a new Sandbox. It's a little tedious and hard to intuitively grasp, but at least there is a way to download and upload an app's source code.

  Downloading the latest source code with `lib get spencer/pocket-to-onedrive -w` will overwrite the existing local folder, including the `.git/` metadata if one was created, making it difficult to even manully maintain a Git copy of an app.
